...
```text
Release ONDEWO NLU Nodejs Client ${ONDEWO_NLU_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO NLU Nodejs Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO NLU Nodejs Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/nlu-client-nodejs`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/nlu-client-nodejs versions --json
```
